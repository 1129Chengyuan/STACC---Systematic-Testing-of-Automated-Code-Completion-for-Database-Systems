...
class TestFindByTemplate(unittest.TestCase):

    def setUp_managers(self):
        self.db = CSVDataTable('managers.csv', {'filepath': 'managers.csv'}, ['playerID'])

    def test_invalid_template_field(self):
        template = {'invalid': 'value'}
        with self.assertRaises(KeyError):
            results = self.db.find_by_template(template)
    ...
    def test_empty_field_list(self):
        ...
        field_list = []
        results = self.db.find_by_template(template, field_list)
...
class Test_CSVDataTable(unittest.TestCase):

    def setUp_Parks(self):
        self.table = CSVDataTable('Parks.csv', {'filename': 'Parks.csv'}, ['park.key'])

    def test_find_by_template_one_match(self):
        template = {'park.key': 'ATL03'}
        results = self.table.find_by_template(template)
        expected = [
            {'park.key': 'ATL03', 'park.name': 'Suntrust Park', 'park.alias': '', 'city': 'Atlanta', 'state': 'GA', 'country': 'US'}]
        self.assertEqual(results, expected)
 ...
 {'city': 'Toledo',
  ...
  'state': 'OH'}]
    ...
    def test_find_by_template_with_field_list(self):
        ...
        results = self.table.find_by_template(template, field_list)
...
class TestCSVDataTable(unittest.TestCase):
    ...
    def setUp_TeamsFranchise(self):
        connect_info = {"filename": "TeamsFranchises.csv"}
        key_columns = ["franchID"]
        self.table = CSVDataTable("TeamsFranchises.csv", connect_info, key_columns)

    def test_find_by_template_one_match(self):
        template = {"franchID": "BOS"}
        expected = [{"franchID": "BOS", "franchName": "Boston Red Sox", "active": "Y"}]
        actual = self.table.find_by_template(template)
 ...
 {'franchID': 'WSN', 'franchName': 'Washington Nationals'}]
        actual = self.table.find_by_template(template, field_list)
        self.assertEqual(actual, expected)
